...
struct RecordingView: View {
    ...
    private var recordingControlButtons: some View {
        if !speechManager.isAuthorized {
            ...
            HStack(spacing: 40) {
                if speechManager.isRecording {
                    ...
                    controlButton(icon: "pause.fill", color: .orange, action: pauseRecording)
                        .opacity(showSaveOptions ? 0 : 1)
                        .scaleEffect(showSaveOptions ? 0.1 : 1)
                    // 停止按钮
            ...
            .animation(.easeInOut(duration: 0.3), value: showSaveOptions)
        }
    }
    ...
    private var saveDiscardButtons: some View {
        HStack(spacing: 60) {
            ...
            optionButton(icon: "xmark", text: "丢弃", color: .red, action: discardRecording)
                .opacity(showSaveOptions ? 1 : 0)
                .scaleEffect(showSaveOptions ? 1 : 0.1)
            // 保存按钮
            optionButton(
                icon: "checkmark", 
                text: "保存", 
            ...
            .scaleEffect(showSaveOptions ? 1 : 0.1)
            .onAppear {
                print("🟣 [DEBUG] 保存按钮显示 - 文字内容: '\(speechManager.transcribedText)'")
                print("🟣 [DEBUG] 保存按钮显示 - 是否为空: \(speechManager.transcribedText.isEmpty)")
                print("🟣 [DEBUG] 保存按钮显示 - 是否禁用: \(speechManager.transcribedText.isEmpty || isSaving)")
            }
        }
        .animation(.easeInOut(duration: 0.3), value: showSaveOptions)
    ...
    private func stopAndFinalize() {
        print("🟢 [DEBUG] RecordingView - 开始停止并完成录音")
        print("🟢 [DEBUG] RecordingView - 停止前文字: '\(speechManager.transcribedText)'")
        print("🟢 [DEBUG] RecordingView - 停止前录音状态: \(speechManager.isRecording)")
        print("🟢 [DEBUG] RecordingView - 停止前暂停状态: \(isPaused)")
        ...
        print("🟢 [DEBUG] RecordingView - 停止后文字: '\(self.speechManager.transcribedText)'")
        print("🟢 [DEBUG] RecordingView - 文字是否为空: \(self.speechManager.transcribedText.isEmpty)")
    }
